...
output_path = os.path.join(source_path, 'questions')
FindJSFiles.main(source_path, output_path)

# 构造combind.js/ combind_sorted.js路径
combined_path = os.path.join(output_path, 'combined.js')
combined_sorted_path = os.path.join(output_path, 'combined_sorted.js')

# 清理html标签
clean_html.main(combined_path)


# 构造清理后路径
base, ext = os.path.splitext(combined_path)
combined_clean_path = f"{base}_clean{ext}"
...
try:
    # 首先尝试打开combined.js
     with open(combined_clean_path, 'r', encoding='utf-8') as file:
            content = file.read()
